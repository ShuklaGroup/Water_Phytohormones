...
for i in range(len(count)):
    if i == 0:
        avg_dist1.append(np.median(dist1[0:sums[i]]))
        err_dist1.append(np.std(dist1[0:sums[i]]))
        avg_dist2.append(np.median(dist2[0:sums[i]]))
        err_dist2.append(np.std(dist2[0:sums[i]]))
    if i > 0:
        avg_dist1.append(np.median(dist1[sums[i-1]:sums[i]]))
        err_dist1.append(np.std(dist1[sums[i-1]:sums[i]]))
        avg_dist2.append(np.median(dist2[sums[i-1]:sums[i]]))
